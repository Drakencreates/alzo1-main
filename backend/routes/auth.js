...
const createModelProxy = (name, mongooseModel) => {
  return new Proxy(mongooseModel, {
    get(target, prop) {
      if (global.useLocalStore && localStore[name]) {
        return localStore[name][prop] || target[prop];
      }
      return target[prop];
    },
    construct(target, args) {
      if (global.useLocalStore && localStore[name]) {
        const ModelClass = localStore[name];
...
const User = createModelProxy('User', require('../models/User'));
const Medication = createModelProxy('Medication', require('../models/Medication'));
const Video = createModelProxy('Video', require('../models/Video'));
const Appointment = createModelProxy('Appointment', require('../models/Appointment'));
const SosAlert = createModelProxy('SosAlert', require('../models/SosAlert'));
const CaregiverPatient = createModelProxy('CaregiverPatient', require('../models/CaregiverPatient'));
const MedicalRecord = createModelProxy('MedicalRecord', require('../models/MedicalRecord'));
